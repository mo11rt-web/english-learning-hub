    ...
    <AppShell requireRole="teacher">
      ...
      {!selectedInquiry ? (
        <div className="flex flex-col gap-3 pb-24">
          {visibleInquiries.map((inquiry) => <button key={inquiry.id} onClick={() => setSelectedId(inquiry.id)} className="text-right"><GlassCard className="hover:ring-2 hover:ring-brand-primary/30 transition-all"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-bold text-brand-text truncate">{inquiry.title}</p><p className="text-sm text-brand-textMuted mt-1">الطالب: {inquiry.studentName} · المجموعة: {groupNamesFor(inquiry)}</p><p className="text-xs text-brand-textMuted mt-1">{inquiry.lessonId && lessons[inquiry.lessonId] ? `الدرس: ${lessons[inquiry.lessonId].title}${lessons[inquiry.lessonId].unitId && units[lessons[inquiry.lessonId].unitId] ? ` · الوحدة: ${units[lessons[inquiry.lessonId].unitId].title}` : ""}` : "منصة عامة"} · {formatDate(inquiry.lastMessageAt)}</p></div><span className={`px-2.5 py-1 rounded-lg text-xs shrink-0 ${statusStyles[inquiry.status]}`}>{statusLabels[inquiry.status]}</span></div></GlassCard></button>)}
