...
export default function AnnouncementsPage() {
  ...
  const resetForm = () => {
    setEditingId(null); setTitle(""); setBody(""); setTargetGroupId(""); setLinkUrl(""); setStartAt(""); setEndAt(""); setStatus("draft"); setFeatured(false); setPublicAnnouncement(false); setImageFile(null); setExistingImageUrl("");
  };

  const uploadImage = async () => {
    if (!imageFile || !user) return existingImageUrl || "";
    if (imageFile.size > 5 * 1024 * 1024) throw new Error("حجم صورة الإعلان يجب ألا يتجاوز 5 ميغابايت.");
    const storageRef = ref(storage, `announcements/${user.uid}/${Date.now()}-${imageFile.name}`);
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        ...(imageUrl ? { imageUrl } : {}), ...(linkUrl.trim() ? { linkUrl: linkUrl.trim() } : {}),
