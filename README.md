# Learn English — with Mohanad Allawi

**تعلّم. احترف. انجح.**

منصة تعليمية متكاملة (Web App / PWA) للأستاذ مهند علاوي لإدارة الطلاب والأقسام
الدراسية، وإنشاء دروس ومفردات مع نطق صوتي مجاني، وواجبات واختبارات مع تصحيح
تلقائي للأسئلة الموضوعية، وتدريب تفاعلي للأفعال الشاذة، ونظام إشعارات حي،
ومتابعة تقدم الطلاب — كل ذلك متصل بقاعدة
...
**اختيار "قسم" (Workspace) عند دخول المعلم — تاسع/بكالوريا أو أي أقسام تضيفها،
مع فصل كامل لكل بيانات القسم (طلاب، دروس، أسئلة، واجبات...) بلا اختلاط** ·
إدارة طلاب كاملة (إنشاء بكلمة مرور تختارها بنفسك، **تعديل**، **حذف ناعم مع
تأكيد واسترجاع**، **نافذة مشاركة بيانات الدخول بعد الإنشاء مع نسخ وزر واتساب
جاهز**) · محرر دروس بكتل محتوى متعددة (عناوين، فقرات عربي/إنجليزي، قواعد،
أمثلة، صور/PDF/صوت مرفوعة مباشرة، فيديو يوتيوب مضمّن، ملف Google Drive مضمّن،
صفحة من الكتاب، مفردات) مع عرض داخل المنصة بالكامل · **نافذة "إضافة درس"
سريعة** (عنوان + رابط فيديو + رفع PDF بحد أقصى 4MB + ملاحظات) · **أسئلة كويز
لكل درس مع تصحيح فوري بإطار أخضر/أحمر وملخص نتيجة وانتقال تلقائي للدرس
التالي** · مكتبة مفردات مع نطق أمريكي/بريطاني وبطاقات Flashcards · بنك أسئلة ·
بنك أسئلة الدورات السابقة (بحث وتصفية) · **مكتبة الأفعال الشاذة كتدريب تفاعلي
(3 أنماط، مستويات صعوبة، نقاط ونتيجة نهائية)** · إنشاء واجبات واختبارات ·
تصحيح تلقائي وصفحة تصحيح للمعلم · نظام نقاط ومستويات (Gamification) · مشاركة
نتائج الطالب مع ولي الأمر برابط آمن بدون تسجيل دخول · **نظام إشعارات حي (جرس +
Badge + حذف فوري عند القراءة) موصول بنشر الدروس والواجبات والإعلانات وتسليم
الطلاب** · **هوية PWA كاملة** (قابلة للتثبيت على الشاشة الرئيسية، أيقونة
مصممة، ألوان تطبيق) · **تنقل سفلي على الهاتف (Bottom Navigation) بذوق تطبيقات
حديثة** · تحسينات لوحة مفاتيح (تركيز تلقائي على أول حقل بكل نافذة، Enter
للحفظ) · لوحة تحكم بإحصائيات حية · تصميم متجاوب بالكامل RTL/LTR.

**مؤجل عمدًا** (لإبقاء هذا الإصدار قابلًا للاختبار والتسليم فعليًا):
مكتبة القواعد والأزمنة المنظمة · استيراد درس من PDF أو رابط ويب بمساعدة
الذكاء الاصطناعي · مساعد الذكاء الاصطناعي للمعلم · تحليل أخطاء الطلاب
التجميعي · التقارير القابلة للتصدير (PDF/Excel) · معلم مساعد بصلاحيات محدودة
(البنية جاهزة في قاعدة البيانات لكن الواجهة غير مبنية بعد) · تدريب النطق عبر
الميكروفون وSpeech-to-Text · الحضور والغياب · Push Notifications عبر
Firebase Cloud Messaging (النظام الحالي إشعارات داخلية فقط، تعمل وقت
استخدام الطالب/المعلم للمنصة فعليًا) · الدروس المباشرة · الشهادات
والاشتراكات · تغيير كلمة مرور حساب موجود من داخل الواجهة (قيد حقيقي من
Firebase، الحل موثّق داخل التطبيق نفسه بزر "🔑 كلمة المرور").
